...
class MovieBudgetCounterProtocol(MessageProtocol):
    def __init__(self):
        super().__init__(
            item_to_bytes=self.__to_movie_budget_counter_pb2,
            encode_all=self.__encode_all,
            bytes_to_item=self.__to_movie_budget_counter,
            decode_all=self.__decode_all,
        )

    def __to_movie_budget_counter_pb2(self, movie_budget_counter: MovieBudgetCounter):
        movie_budget_counter_encoded = movie_budget_counter_pb2.MovieBudgetCounter()
    ...
    def __encode_all(self, a_list):
        return movie_budget_counter_pb2.MovieBudgetCounters(
            list=a_list
        ).SerializeToString()
    ...
    def __decode_all(self, buf: bytes, bytes_amount: int):
        pb2_list = movie_budget_counter_pb2.MovieBudgetCounters()
