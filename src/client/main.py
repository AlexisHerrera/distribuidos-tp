...
def count_lines_in_file(
    file_object: io.TextIOWrapper, file_description: str, file_path_for_msg: str
) -> bool:
...
def send_movies(client_socket):
    ...
    try:
        ...
        logging.info('Finished sending all batches')
        client_socket.close()
...
def main():
    ...
    for file_obj, description, path_name in files_to_process:
        if not count_lines_in_file(file_obj, description, path_name):
            ...
            print(
                f'There was a problem processing the {description} file. '
